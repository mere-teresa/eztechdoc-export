...
recommendation engine, powered by [YOOCHOOSE](https://yoochoose.com/).
It allows you to track the way visitors use your website and suggests
recommended content to them based on their behavior.
...
**3.** Import additional routing by adding following lines to your
`routing.yml` file:
...
<div class="table-wrap">
  ...
  ---------------- -------------------------------------------------------
  yoochoose.custom Your YOOCHOOSE customer ID.
  er\_id           

  yoochoose.licens Your YOOCHOOSE license key.
  e\_key           

  server\_uri      The URI your site’s REST API can be accessed from.

  recommender.incl Content Types on which the tracking script will be
  uded\_content\_t shown. See [Tracking](#Recommendation-Tracking)below
...
While Symfony `dev` environment keeps track of changes to yml
...
parameter (see the `default_settings.yml` file delivered with this
bundle). Here you can define for which Content Types tracking script
will be shown.
...
[tracking in
general](https://doc.yoochoose.net/display/PUBDOC/1.+Tracking+Events)
and about the [generic asynchronous javascript
tracker](https://doc.yoochoose.net/display/PUBDOC/Tracking+with+yc.js).

</div>
</div>
Additionally, in case of missing content owner ID, there’s an option
...
<div class="table-wrap">
  ...
  r       
  ---- -- -----------------------------------------------------------------
  cont in In content-based views the Twig variable holding the content ID
  entI t  (the content you want to get recommendations for).
  d       

  scen st Scenario used to display recommendations. You can create custom
  ario ri scenarios at the YOOCHOOSE dashboard.
       ng 

  limi in Number of recommendations to show.
  t    t  

  cont st Content Types you are expecting in response.
  entT ri 
  ype  ng 

  temp st HandleBars template name (your templates are stored in
  late ri the`ezsystems/recommendation-bundle/Resources/public/views`
       ng directory. Take a look at `default.html.twig` which includes a
          default template that can be used to prepare customized
          versions).

  fiel ar Fields which are required and will be requested from the
  ds   ra recommender engine. These field names are also used inside
       y  HandleBars templates.
  -------------------------------------------------------------------------
...
channel](http://symfony.com/doc/current/cookbook/logging/channels_handlers.html).
...
Service](https://doc.yoochoose.net/display/PUBDOC/Developer+Guide)
